    ...
    <div class="flex flex-col md:flex-row gap-4">
        ...
        <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
          <table class="w-full">
            ...
            <tbody>
              ...
              {{-- @php
            //   dd($cart_items);
                $price = collect($item['unit_amount'] ?? [])->firstWhere('customer_id', $selectedCustomer);
                dd($price);
              @endphp --}}
              <tr wire:key="{{$item['item_id']}}">
                ...
                <td class="py-4">{{ Number::currency($item->prices->first()->price ?? 'N/A', 'AED') }}</td>
                <td class="py-4">
                  <div class="flex items-center">
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <button class="bg-blue-500 text-white py-2 px-4 rounded-lg mt-4 w-full">Place Order</button>
        </div>
      </div>
    </div>
  </div>
